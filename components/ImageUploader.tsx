...
const ImageUploader: React.FC<ImageUploaderProps> = ({ 
  ...
  return (
    <div className="bg-pink-200 p-6 rounded-2xl shadow-md text-zinc-900">
      {imageToCrop && (
        <ImageCropperModal
          isOpen={!!imageToCrop}
      ...
      <div 
        className={`relative flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-xl transition-colors duration-300 ${isDragging ? 'border-pink-400 bg-pink-100/80' : 'border-pink-300 bg-pink-100/50'} ${!preview ? 'cursor-pointer hover:border-pink-400' : ''}`}
        ...
        {preview ? (
          <div className="flex flex-col items-center text-center">
            ...
            <div className="flex gap-2">
                <button onClick={triggerFileInput} className="text-sm font-medium text-white bg-pink-500 hover:bg-pink-600 px-4 py-2 rounded-lg transition-colors">
                    Thay đổi ảnh
                </button>
            </div>
          </div>
      ...
      <div className="mt-4 flex flex-col items-center justify-center gap-3">
        ...
        <label htmlFor="face-lock-toggle" className="flex items-center cursor-pointer">
          <div className="relative">
            ...
            />
            <div className={`block w-14 h-8 rounded-full transition-colors ${isFaceLockEnabled ? 'bg-pink-500' : 'bg-pink-300'}`}></div>
        ...
        <label htmlFor="with-flowers-toggle" className="flex items-center cursor-pointer">
          <div className="relative">
            ...
            />
            <div className={`block w-14 h-8 rounded-full transition-colors ${withFlowers ? 'bg-pink-500' : 'bg-pink-300'}`}></div>
