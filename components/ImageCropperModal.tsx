  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden p-4 md:p-6">
        <h3 className="text-xl font-bold text-orange-500 mb-4 text-center">Tùy chỉnh ảnh gốc</h3>
        <div className="flex-grow min-h-0 flex items-center justify-center bg-pink-50 rounded-lg p-2">
           {imgSrc && (
             <ReactCrop
                crop={crop}
            ...
            {aspectRatios.map(ar => (
                <button
                    ...
                    className={`px-3 py-1.5 text-sm font-medium rounded-full border transition-all duration-200
                        ${aspect === ar.value ? 'bg-pink-500 border-pink-500 text-white' : 'bg-pink-100 border-pink-200 text-pink-800 hover:bg-pink-200'}
                    `}
                >
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button onClick={handleConfirmCrop} className="w-full sm:w-auto bg-pink-500 text-white font-bold py-3 px-6 rounded-xl hover:bg-pink-600 transition-colors duration-200">
                Xác nhận
            </button>
        </div>
      </div>
