...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, images, initialIndex, prompt }) => {
    ...
    const handleDownload = () => {
        ...
        if (isMobile) {
            ...
            link.href = image.url;
            link.download = `nghe-thuat-sang-chanh-${image.id}.jpg`;
    ...
    const handleShare = async () => {
        try {
            ...
            const file = new File([blob], `nghe-thuat-sang-chanh-${image.id}.jpg`, { type: blob.type });

            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: 'Nghệ Thuật Sang Chảnh',
                    text: 'Ảnh được tạo bởi App Nghệ Thuật Sang Chảnh.',
                });
            } else {
    ...
    return (
        ...
        >
            ...
            >
                <div className="flex-shrink-0 md:w-2/3 bg-pink-50 flex items-center justify-center p-4 relative overflow-hidden allow-context-menu">
                ...
                <div className="flex flex-col p-6 w-full md:w-1/3 text-zinc-900">
                    <div className="flex justify-between items-start">
                        <h3 className="text-xl font-bold text-orange-500">Chi tiết ảnh</h3>
                    ...
                    <div className="mt-4 space-y-4 flex-grow overflow-y-auto pr-2">
                        <div>
                            <h4 className="text-sm font-semibold text-pink-600">Prompt</h4>
                            <p className="text-xs text-zinc-700 bg-pink-100 p-2 rounded-md mt-1 break-words">{prompt}</p>
                        </div>
                        <div>
                            <h4 className="text-sm font-semibold text-pink-600">Seed</h4>
                            <p className="text-xs text-zinc-700 bg-pink-100 p-2 rounded-md mt-1 break-words">{image.seed}</p>
                        </div>
                    </div>
                    <div className="mt-6 space-y-3">
                        ...
                        <button onClick={handleShare} className="w-full bg-pink-500 text-white font-bold py-3 px-4 rounded-xl hover:bg-pink-600 transition-colors duration-200 flex items-center justify-center gap-2">
                           <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
